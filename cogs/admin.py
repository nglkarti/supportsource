...
class Admin(commands.Cog):
    ...
    async def amiadmin(self, ctx: Context[BotT]):
        ...
        if ctx.author.id in self.config["owners"]:
            return await ctx.send(f"Yes, **{ctx.author.name}** \nYou did make me while Erin was asleep.")

        
        if ctx.author.id == 952329345781858424:
            return await ctx.send(f"Not really **{ctx.author.name}**.. stop it henry.")

        await ctx.send(f"No your not, {ctx.author.name}")
